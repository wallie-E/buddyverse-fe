...
import { getMyExchanges, getExchangeInfo, confirmExchange } from '../api/wechatExchange';
  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      ...
      case 'wechat_exchange_confirmed':
        return <MessageCircleMore className="h-6 w-6 text-green-600" />;
      default:
        ...
        <div className="bg-white rounded-2xl shadow-sm mb-8">
          <div className="flex">
            {[
              ...
              { key: 'wechat', label: '微信交换', count: exchangeRecords.length }
            ].map((tab) => (
              <button
                key={tab.key}
            ...
            <div className="space-y-4">
              {exchangeRecords.map((record) => (
                <div
                  ...
                  className={`bg-white rounded-2xl shadow-sm p-6 hover:shadow-lg transition-all duration-300 border border-gray-100 ${record.status !== 1 ? 'cursor-pointer' : 'cursor-default'}`}
                >
                  <div className="flex items-start space-x-4">
                    {/* Icon */}
                    <div className="flex-shrink-0 mt-1">
                      <div className="w-12 h-12 rounded-2xl bg-gradient-to-r from-green-100 to-green-200 flex items-center justify-center">
                        ...
                        <div className="flex-1">
                          ...
                          {record.status === 1 && record.otherWechat && (
                            <div className="bg-gray-50 rounded-lg p-3 mb-2">
                              <p className="text-sm text-gray-600 mb-1">对方的微信号：</p>
                              <p className="text-base font-semibold text-gray-900">{record.otherWechat}</p>
                            </div>
                          )}
